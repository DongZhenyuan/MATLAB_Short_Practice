...
默认情况下，训练开始的学习率等于 10-3 (--lr)
在 epochs 的数量超过第一个 milestone (--milestone)之后, lr 除以 100
直至这一节点, 则采用 FFDNet 论文中所描述的正交化技术(--no_orthog to set it off).
...
import warnings
warnings.filterwarnings('ignore')  # 忽视警告可不是好习惯，这是个反面教材
...
def main(args):
	...
	loader_train = DataLoader(dataset=dataset_train, num_workers=6, batch_size=args.batch_size, shuffle=False)  # 改
	...
	for epoch in range(start_epoch, args.epochs):
		...
		try:
			if epoch == 0:
				...
				for idx in range(2):
					imclean = utils.make_grid(img_val.data[idx].clamp(0., 1.), nrow=2, normalize=False, scale_each=False)
					imnsy = utils.make_grid(imgn_val.data[idx].clamp(0., 1.), nrow=2, normalize=False, scale_each=False)
			...
			for idx in range(2):
				imrecons = utils.make_grid(out_val.data[idx].clamp(0., 1.), nrow=2, normalize=False, scale_each=False)
				writer.add_image('Reconstructed validation image {}'.format(idx), imrecons, epoch)
			# Log training images
			imclean = utils.make_grid(img_train.data, nrow=8, normalize=True, scale_each=True)
		...
		save_dict = {'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict(), 'training_params': training_params, 'args': args}
